...
describe('test using an api_key', function() {
  var bzClient;
  before(function() {
    bzClient = bz.createClient({
      api_key: authConfig.api_key,
      url: authConfig.auth.url,
    });
  });

  it('tests changing a bug using only an api_key', function(done) {
    let url = authConfig.auth.url.split('/rest')[0];
    bzClient.updateBug('27114', {url: +'/show_bug.cgi?id=27114'}, function(err, result) {
      if (err) throw err;
      assert.ok(true);
      done();
    });
