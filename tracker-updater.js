...
bws_tracker.searchBugs( searchParams, function( error, bugs ) {
	...
	for ( let i = 0; i < bugs.length; i++ ) {
		...
		community_tracker.getBug(bws_bug.cf_community_bug, function(error, community_bug) {
			...
			if ( bws_status != community_status ) {
				bws_tracker.updateBug( bws_bug.id, { cf_community_status: community_status }, function( error, ok ) {
					console.log(`Updating BWS Tracker Bug ${bws_bug.id} with status ${bws_status} to community status ${community_status} from community bug ${community_bug.id}`);
					if ( error ) {
